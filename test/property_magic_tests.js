...
beforeEach(async function () {
    ...
    theSurvivor = new CPlaybook({
        playbook: 'The Survivor',
        name: "Max",
...
describe(`family view crud tests`, () => {
    ...
    it("process string array commands", async () => {
        ...
        args = {
            property_action: actions.REMOVE,
            property_name: 'traditions', property_value: "Aquatic Cyborgs"
        };
    ...
    it("can figure out int versus string scalar types", async () => {
        let characterProps = PropertyMagic.CharacterProperties();
        assert.ok(characterProps['name']);
        let prop = characterProps["name"];
        assert.strictEqual(prop.name, "name" );
        assert.strictEqual(prop.isa, "String" );
        assert.strictEqual(prop.default, null );

        //check a string array
        prop = characterProps["gear"];
        assert.ok( prop );
        assert.strictEqual(prop.name, "gear" );
        assert.strictEqual(prop.isa, "Array" );

        let familyProps = PropertyMagic.FamilyProperties();
        assert.ok(familyProps['name']);
        prop = familyProps["name"];
        assert.strictEqual(prop.name, "name" );
        assert.strictEqual(prop.isa, "String" );
        assert.strictEqual(prop.default, null );

        //check a string array
        prop = familyProps["moves"];
        assert.ok( prop );
        assert.strictEqual(prop.name, "moves" );
        assert.strictEqual(prop.isa, "Array" );
